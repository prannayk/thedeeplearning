...
class DCGAN():
	...
	def learningR(self):
		return self.learning_rate_1 , self.learning_rate_2, self.momentum
	...
	def build_model(self):
		with tf.device("/gpu:0"):
			...
			d_cost = self.log_loss(real_value - fake_value,True) + penalty
			g_cost = self.log_loss(fake_value, True)
...
# optimizers
# with tf.device("/gpu:0"):
lr1, lr2, momentum = gan.learningR()
...
def save_visualization(X, nh_nw=[batch_size,frames], save_path='../results/%s/sample.jpg'%(sys.argv[4])):
	X = morph(X,frames)
...
for ep in range(epoch):
	...
	save_visualization(gen_samples,save_path=('../results/%s/sample_%d.jpg'%(sys.argv[4],ep)))
